...
const AddCollaboratorsModal = ({ newCollaboratorsModalOpen, toggleNewCollaboratorsModal,allUsers, params }) => {
    ...
    const fetchUsersAndCollaborators = useCallback(async () => {
        try {
            setLoading(true);
            const collaboratorsResponse = await axios.post('/api/get-collaborators', { workspaceId });
    ...
    const handleAddCollaborator = useCallback(async (user) => {
    ...
    return (
        <div className={`fixed z-10 inset-0 overflow-y-auto ${newCollaboratorsModalOpen ? '' : 'hidden'}`}>
            ...
            <div className="flex items-center justify-center min-h-screen p-4">
                <div ref={modalRef} className="bg-white rounded-lg p-6 shadow-lg relative w-full max-w-lg">
                    <button
                    ...
                    <h2 className="text-xl font-bold mb-4 text-center">Add New Collaborators</h2>
                    {showDuplicateWarning && (
                        <p className="text-red-500 mb-2 text-center">This collaborator is already added.</p>
                    )}
