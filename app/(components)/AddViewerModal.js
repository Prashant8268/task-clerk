...
const AddViewerModal = ({ newViewsModalOpen, toggleNewViewsModal, params, allUsers }) => {
    ...
    const fetchUsersAndViewers = useCallback(async () => {
        try {
            setLoading(true);
            const viewerResponse = await axios.post('/api/get-viewers', { workspaceId });
    ...
    const handleAddView = useCallback(async (view) => {
        if (!views.includes(view)) {
            try {
                setLoading(true);
                setShowNotification(false);
                const response = await axios.post('/api/add-viewer', { userEmail: view, workspaceId });
            ...
            <div className="flex items-center justify-center min-h-screen p-4">
                <div ref={modalRef} className="bg-white rounded-lg p-6 shadow-lg relative w-full max-w-lg">
                    ...
                    {showDuplicateWarning && (
                        <p className="text-red-500 mb-2 text-center">This viewer is already added.</p>
                    )}
